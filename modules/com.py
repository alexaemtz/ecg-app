...
class ComunicationGUI(ctk.CTkFrame):
    def __init__(self, root):
        ...
        self.bpm_font = ctk.CTkFont(family="Poppins SemiBold", size=20)
        ...
        self.label_com = ctk.CTkLabel(com_frame, text="Dispositivos disponibles:", anchor="w", font=self.label_font)
        ...
        self.ax.set_title("Electrocardiograma - Monitor en tiempo real - 1D Pro")
        ...
        # Agregar BPM label
        self.bpm_label = ctk.CTkLabel(self.graph_frame, text="BPM: --", font=self.bpm_font, text_color="white")
        self.bpm_label.pack(side="right", padx=20)
        ...
        self.frame_filter.pack(pady=10, padx=10, anchor="w")

        # Activar filtros por defecto
        self.low_pass_var = ctk.StringVar(value="on")  # Cambiado de "off" a "on"
        self.low_pass_toggle = ctk.CTkCheckBox(self.frame_filter, text="Pasa-bajas (20Hz)", variable=self.low_pass_var, onvalue="on", offvalue="off", font=self.text_font)
        self.low_pass_toggle.pack(side="left", padx=10)

        self.high_pass_var = ctk.StringVar(value="on")  # Cambiado de "off" a "on"
        self.high_pass_toggle = ctk.CTkCheckBox(self.frame_filter, text="Pasa-altas (0.5Hz)", variable=self.high_pass_var, onvalue="on", offvalue="off", font=self.text_font)
        self.high_pass_toggle.pack(side="left", padx=10)

        self.notch_var = ctk.StringVar(value="on")  # Cambiado de "off" a "on"
        self.notch_toggle = ctk.CTkCheckBox(self.frame_filter, text="Notch (60Hz)", variable=self.notch_var, onvalue="on", offvalue="off", font=self.text_font)
        self.notch_toggle.pack(side="left", padx=10)

        # Botón para verificar estado de filtros
        self.filter_status_button = ctk.CTkButton(
            self.frame_filter, 
            text="Estado Filtros", 
            command=self.show_filter_status,
            font=self.text_font,
            width=120
        )
        ...
        self.control_frame.pack(pady=10, padx=10, anchor="w")

        self.icon_start = ctk.CTkImage(Image.open("assets/icons/play.png"), size=self.icon_size)
        self.start_button = ctk.CTkButton(self.control_frame, text="Empezar", command=self.start_animation, font=self.text_font, image=self.icon_start)
        self.start_button.pack(side="left", padx=10)

        self.icon_pause = ctk.CTkImage(Image.open("assets/icons/pause.png"), size=self.icon_size)
        self.pause_button = ctk.CTkButton(self.control_frame, text="Pausar", command=self.pause_animation, font=self.text_font, image=self.icon_pause)
        self.pause_button.pack(side="left", padx=10)

        self.icon_stop = ctk.CTkImage(Image.open("assets/icons/stop.png"), size=self.icon_size)
        self.stop_button = ctk.CTkButton(self.control_frame, text="Detener", command=self.stop_animation, font=self.text_font, image=self.icon_stop)
        self.stop_button.pack(side="left", padx=10)

        # ---------- Guardar CSV ----------
        self.save_frame = ctk.CTkFrame(self.main_layout, fg_color="transparent")
        self.save_frame.pack(pady=10, padx=10, anchor="w")

        self.patient_name_entry = ctk.CTkEntry(self.save_frame, placeholder_text="Nombre del paciente", width=200, height=30, font=self.text_font)
        self.patient_name_entry.pack(side="left", padx=10)
        self.patient_id_entry = ctk.CTkEntry(self.save_frame, placeholder_text="Curp del paciente", width=300, height=30, font=self.text_font)
        self.patient_id_entry.pack(side="left", padx=10)
        
        self.icon_save = ctk.CTkImage(Image.open("assets/icons/save.png"), size=self.icon_size)
        self.start_recording_button = ctk.CTkButton(self.save_frame, text="Guardar datos en CSV", command=self.start_continuos_recording, font=self.text_font, image=self.icon_save)
        self.start_recording_button.pack(side="left", padx=10)
        
        self.icon_stop = ctk.CTkImage(Image.open("assets/icons/stop.png"), size=self.icon_size)
        self.stop_recording_button = ctk.CTkButton(self.save_frame, text="Detener guardado", command=self.stop_continuos_recording, font=self.text_font, image=self.icon_stop)
        self.stop_recording_button.pack(side="left", padx=10)
        
        self.recording_status_label = ctk.CTkLabel(self.save_frame, text=" ", font=self.text_font)
    ...
    def begin_device(self):
        ...
        if self.arduino is None:
            try:
                self.port = self.com_port_combobox.get()
                self.baud = int(self.baud_combobox.get())
                self.arduino = serial.Serial(port=self.port, baudrate=self.baud, timeout=1)
    ...
    def reading(self, signal):
        """Lee datos del dispositivo Arduino y devuelve el voltaje en tiempo real"""
        self.arduino.write(signal.encode())
        time.sleep(0.00001)
        try:
            data = self.arduino.readline().decode().strip()
            return float(data) if data else 0  # Devuelve directamente el voltaje
        except:
            return 0
